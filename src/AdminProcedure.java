...
class AdminProcedure {	
	...
	boolean selectItems(int operation){
		...
		switch (selection) {
			...
			case 1 : {
				...
				int displayIndex = 1;
				try {
					rs = libSys.executeSQL(query.SQLShowAllBks);
			...
			case 2 : {
				...
				int displayIndex = 1;
				try {
					rs = libSys.executeSQL(query.SQLShowAllAus);
					libSys.unholdExecuteSQL();
					while (rs.next()) {
			...
			case 3: {
				...
				int displayIndex = 1;
				try {
					rs = libSys.executeSQL(query.SQLShowAllPbs);
					libSys.unholdExecuteSQL();
					while (rs.next()) {
			...
			case 4: {
				...
				int displayIndex = 1;
				try {
					rs = libSys.executeSQL(query.SQLShowAllBch);//!
					libSys.unholdExecuteSQL();
					while (rs.next()) {
			...
			case 5: {
				...
				int displayIndex = 1;
				try {
					rs = libSys.executeSQL(query.SQLShowAllBrw);
					libSys.unholdExecuteSQL();
					while (rs.next()) {
			...
			switch (action) {
				...
				case 1: {
					...
					while (true) {
						...
						try {
							rs = libSys.executeSQL(para, query.SQLCheckBkName);
							libSys.unholdExecuteSQL();
							if (rs.next()) {
						...
						changeData(data, query.SQLInsertBkName);
						...
						changeData(data, query.SQLInsertBkAu);					
						break;
					}
					break;
				}
				...
				case 2: {
					...
					while (true) {
						...
						if (updateBoth.equalsIgnoreCase("y")) {
							...
							changeData(data, query.SQLUpdateBkName2);
						} else {
							data.add(updBookName);
							data.add(admin.bookID);
							changeData(data, query.SQLUpdateBkName);
						}
						...
						data.add(authorIDs.get(selection - 1));
						data.add(bookID);
						changeData(data, query.SQLUpdateBkAu);	
						break;
					}
					break;
				}
				...
				case 3:{
					List<String> data = new ArrayList<>();
					data.add(admin.bookID);
					changeData(data, query.SQLDeleteBkName);
					break;
				}
				//add author name
				case 4: {
					...
					while (true) {
						...
						para.add(newAuName);
						try {
							rs = libSys.executeSQL(para, query.SQLCheckAuName);
							libSys.unholdExecuteSQL();
							if (rs.next()) {
						...
						List<String> data = new ArrayList<>();
						data.add(newAuName);
						changeData(data, query.SQLInsertAuName);
						break;
					}
					break;
				}
				...
				case 5: {
					...
					while (true) {
						...
						changeData(data, query.SQLUpdateAuName);
						break;
					}
					break;
				}
				...
				case 6: {
					List<String> data = new ArrayList<>();
					data.add(admin.authorID);
					changeData(data, query.SQLDeleteAuName);
					break;
				}
				//add publisher name
				case 7: {
					...
					while (true) {
						...
						para.add(newPubAddr);
						try {
							rs = libSys.executeSQL(para, query.SQLCheckPbName);
							libSys.unholdExecuteSQL();
							if (rs.next()) {
						...
						changeData(data, query.SQLInsertPbName);//!
						break;
					}
					break;
				}
				...
				case 8: {
					...
					while (true) {
						...
						changeData(data, query.SQLUpdatePbName);
						break;
					}
					break;
				}
				...
				case 9: {
					List<String> data = new ArrayList<>();
					data.add(admin.publisherID);
					changeData(data, query.SQLDeletePbName);
					break;
				}
				//add branch name
				case 10: {
					...
					while (true) {
						...
						para.add(newBchAddr);
						try {
							rs = libSys.executeSQL(para, query.SQLCheckBchName);
							libSys.unholdExecuteSQL();
							if (rs.next()) {
						...
						data.add(newBchName);
						data.add(newBchAddr);
						changeData(data, query.SQLInsertBchName);
						break;
					}
					break;
				}
				...
				case 11: {
					...
					while (true) {
						...
						changeData(data, query.SQLUpdateBchName);
						break;
					}
					break;
				}
				...
				case 12: {
					List<String> data = new ArrayList<>();
					data.add(admin.branchID);
					changeData(data, query.SQLDeleteBchName);
					break;
				}
				//add borrower name
				case 13: {
					...
					changeData(data, query.SQLInsertBrwName);
					break;
				}
				//update borrower name
				case 14: {
					...
					changeData(data, query.SQLUpdateBrwName);
					break;
				}
				//delete borrower name
				case 15: {
					List<String> data = new ArrayList<>();
					data.add(admin.borrowerID);
					changeData(data, query.SQLDeleteBrwName);
					break;
				}
			}
	...
	boolean changeDueDate(){
		...
		try {
			rs = libSys.executeSQL(query.SQLShowAllLoan);
			libSys.unholdExecuteSQL();
			while (rs.next()) {
				String bookID = rs.getString("bookId");
				bookIDs.add(bookID);
			...
			data.add(admin.branchID);
			data.add(admin.borrowerID);
			changeData(data, query.SQLUpdateDueDate);
		return true;
	}
